-- ==============================================
-- INSERT CUSTOMERS FROM PRICING DATA
-- This script adds unique client-branch combinations
-- from the provided pricing data to the customers table
-- ==============================================

-- ==============================================
-- INSERT CUSTOMERS FROM PRICING DATA
-- This script inserts all client-branch-SKU combinations
-- ==============================================
--
-- IMPORTANT: Before running, check your unique constraint:
-- Run: sql/utilities/CHECK_CUSTOMERS_CONSTRAINT.sql
--
-- If you get an error about "no unique or exclusion constraint matching",
-- try one of these alternatives:
-- 1. Use: sql/utilities/INSERT_CUSTOMERS_FROM_PRICING_DATA_NO_CONFLICT.sql (if no constraint)
-- 2. Or modify the ON CONFLICT line below to match your constraint
--
-- Common constraint names:
-- - customers_client_branch_sku_unique (for UNIQUE(client_name, branch, sku))
-- - customers_client_name_branch_key (for UNIQUE(client_name, branch))
...
WITH pricing_data AS (
  SELECT * FROM (VALUES 
    ...
    -- Data from 11/15/2025 (this will update Intercity Bachupally with new price)
...
-- Using constraint name (verified: customers_client_branch_sku_unique)
...
-- Alternative: Can also use column list instead of constraint name
-- ON CONFLICT (client_name, branch, sku) DO UPDATE SET ...
